...
class EditProfileActivity : AppActivity() {
    ...
    override fun initData() {
        ...
        binding.ivEyeGender.setOnClickListener {
            isGender = if (isGender == EditProfileConstant.ENABLE) {
                binding.ivEyeGender.setImageResource(R.drawable.ic_eye_off_edit_profile)
                EditProfileConstant.DISABLE
            } else {
                binding.ivEyeGender.setImageResource(R.drawable.ic_eye_profile)
                EditProfileConstant.ENABLE
            }
            enableButtonSave()
        }

        binding.ivEyePhone.setOnClickListener {
            isPhone = if (isPhone == EditProfileConstant.ENABLE) {
                binding.ivEyePhone.setImageResource(R.drawable.ic_eye_off_edit_profile)
                EditProfileConstant.DISABLE
            } else {
                binding.ivEyePhone.setImageResource(R.drawable.ic_eye_profile)
                EditProfileConstant.ENABLE
            }
            enableButtonSave()
        }
        ...
        binding.ivEyeBirthday.setOnClickListener {
            isBirthday = if (isBirthday == EditProfileConstant.ENABLE) {
                binding.ivEyeBirthday.setImageResource(R.drawable.ic_eye_off_edit_profile)
                EditProfileConstant.DISABLE
            } else {
                binding.ivEyeBirthday.setImageResource(R.drawable.ic_eye_profile)
                EditProfileConstant.ENABLE
            }
            enableButtonSave()
        }
        ...
        binding.ivEyeEmail.setOnClickListener {
            isEmail = if (isEmail == EditProfileConstant.ENABLE) {
                binding.ivEyeEmail.setImageResource(R.drawable.ic_eye_off_edit_profile)
                EditProfileConstant.DISABLE
            } else {
                binding.ivEyeEmail.setImageResource(R.drawable.ic_eye_profile)
                EditProfileConstant.ENABLE
            }
            enableButtonSave()
        }
        ...
        binding.ivAddress.setOnClickListener {
            isStreet = if (isStreet == EditProfileConstant.ENABLE) {
                binding.ivAddress.setImageResource(R.drawable.ic_eye_off_edit_profile)
                EditProfileConstant.DISABLE
            } else {
                binding.ivAddress.setImageResource(R.drawable.ic_eye_profile)
                EditProfileConstant.ENABLE
            }
            enableButtonSave()
        }
...
//        callProfileCustomer()
    ...
    private fun dataIsShow() {
        ...
        if (isGender == EditProfileConstant.DISABLE) {
            binding.ivEyeGender.setImageResource(R.drawable.ic_eye_off_edit_profile)
        } else {
            binding.ivEyeGender.setImageResource(R.drawable.ic_eye_profile)
        }

        if (isBirthday == EditProfileConstant.DISABLE) {
            binding.ivEyeBirthday.setImageResource(R.drawable.ic_eye_off_edit_profile)
        } else {
            binding.ivEyeBirthday.setImageResource(R.drawable.ic_eye_profile)
        }

        if (isEmail == EditProfileConstant.DISABLE) {
            binding.ivEyeEmail.setImageResource(R.drawable.ic_eye_off_edit_profile)
        } else {
            binding.ivEyeEmail.setImageResource(R.drawable.ic_eye_profile)
        }

        if (isPhone == EditProfileConstant.DISABLE) {
            binding.ivEyePhone.setImageResource(R.drawable.ic_eye_off_edit_profile)
        } else {
            binding.ivEyePhone.setImageResource(R.drawable.ic_eye_profile)
        }

        if (isStreet == EditProfileConstant.DISABLE) {
            binding.ivAddress.setImageResource(R.drawable.ic_eye_off_edit_profile)
        } else {
            binding.ivAddress.setImageResource(R.drawable.ic_eye_profile)
        }
    ...
    private fun setDataOld() {
        ...
        oldDistrictId = districtId
        oldDistrictName = districtName
        oldWardId = wardId
        oldWardName = wardName

        oldIsCheckNickname = isNickName
        oldIsGender = isGender
        oldIsPhone = isPhone
        oldIsBirthday = isBirthday
        oldIsEmail = isEmail
        oldIsStreet = isStreet
        enableButtonSave()
    }
    ...
    private fun enableButtonSave() {
        ...
        val isUseNickname = if (binding.rbCheckedNickName.isVisible) 1 else 0
        ...
        val recentCity = cityId
        val recentDistrict = districtId
        val recentWard = wardId
        if (oldUsername != recentUsername ||
            ...
            oldCityId != recentCity ||
            oldDistrictId != recentDistrict ||
            oldWardId != recentWard ||
            oldIsCheckNickname != isUseNickname ||
            oldIsGender != isGender ||
            oldIsPhone != isPhone ||
            oldIsBirthday != isBirthday ||
            oldIsEmail != isEmail ||
